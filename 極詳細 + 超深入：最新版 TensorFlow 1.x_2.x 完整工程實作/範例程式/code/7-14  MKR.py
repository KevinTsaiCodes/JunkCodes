...
class CrossCompressUnit(base.Layer):
    ...
    def _call(self, inputs):
        v, e = inputs#v和e的形狀為[batch_size, dim]
        v = tf.expand_dims(v, dim=2)#v的形狀為 [batch_size, dim, 1]
        e = tf.expand_dims(e, dim=1)#e的形狀為 [batch_size, 1, dim]

        c_matrix = tf.matmul(v, e)#c_matrix的形狀為 [batch_size, dim, dim]
        c_matrix_transpose = tf.transpose(c_matrix, perm=[0, 2, 1])

        c_matrix = tf.reshape(c_matrix, [-1, self.dim])#c_matrix的形狀為[batch_size * dim, dim]
        ...
        v_output = tf.reshape( self.f_vv(c_matrix) + self.f_ev(c_matrix_transpose),
                              [-1, self.dim]) + self.bias_v#v_output的形狀為[batch_size, dim]
...
class MKR(object):
    ...
    def _build_low_layers(self, args):
        ...
        #取得指定輸入對應的詞內嵌，形狀為[batch_size, dim]
    ...
    def _build_high_layers(self, args):
        #RS模型的處理
        use_inner_product = True #指定相似度分數計算的模式
        if use_inner_product:    #內積模式
            #self.scores的形狀為[batch_size]
            self.scores = tf.reduce_sum(self.user_embeddings * self.item_embeddings, axis=1)
        else:
            #self.user_item_concat的形狀為[batch_size, dim * 2]
            ...
            self.vars_rs.extend(rs_pred_mlp.variables)  #收集參數
        self.scores_normalized = tf.nn.sigmoid(self.scores)

        #KGE模型的處理
        #self.head_relation_concat的形狀為[batch_size, dim * 2]
    ...
    def _build_loss(self, args):
        ...
        for var in self.vars_kge: #計算L2正則
