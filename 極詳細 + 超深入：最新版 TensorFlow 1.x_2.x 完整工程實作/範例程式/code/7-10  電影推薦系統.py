...
users_from_idx = sorted(set(r.user_id for r in ratings), key=int)#使用者ID
...
items_from_idx = sorted(set(r.item_id for r in ratings), key=int)#電影ID
...
sess = tf.InteractiveSession()#將id 與電影交叉。評分填入
...
print('評論最多的使用者{}: {:,d}'.format(user_id, n_ratings))#userid為 547:評論了 2,391次。是最多的

r = next(r for r in reversed(ratings) if r.user_id == user_id and r.rating == 5.0)#找一條評論為5的資料
print('該使用者最後一條5分記錄 {}:\n'.format(user_id))
print(r)

#根據該使用者和電影的索引在分解矩陣中取值，並計算模型的準確度
...
p = np.dot(u, v)
print('Approx. rating: {:,.3f}, diff={:,.3f}, {:,.3%}'.format(p, r.rating - p, p/r.rating))#預測結果

#推薦排名
V = item_factors
user_P = np.dot(V, u)
print('預測出使用者所有的評分:', user_P.shape)

user_items = set(ur.item_id for ur in ratings if ur.user_id == user_id)#該使用者評論的電影
...
user_ranking = [(item_id, p) for item_id, p in user_ranking_raw if item_id not in user_items]#找到該使用者沒有評論過的所有電影評分

top10 = user_ranking[:10]#取出前10個

print('Top 10 items:\n')
for k, (item_id, p) in enumerate(top10):  #得到該使用者喜歡電影的排名
