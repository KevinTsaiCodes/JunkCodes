...
class Position_Embedding(keras.layers.Layer):    
    ...
    def call(self, x):
        ...
        #按照x的1維度累計求和，與arange一樣，產生序列。只不過按照x的實際長度來
...
class Attention(keras.layers.Layer):
    def __init__(self, nb_head, size_per_head, **kwargs):
        ...
        self.output_dim = nb_head*size_per_head#輸出的總維度
    ...
    def Mask(self, inputs, seq_len, mode='mul'):#按照seq_len實際長度對inputs進行計算
        if seq_len == None:
            return inputs
        else:
    ...
    def call(self, x):
        ...
        if len(x) == 3:#解析傳入的入Q_seq,K_seq,V_seq
            Q_seq,K_seq,V_seq = x
            Q_len,V_len = None,None
        elif len(x) == 5:#Q_len,V_len為mask的長度
            Q_seq,K_seq,V_seq,Q_len,V_len = x
            
        print("Q_seq",Q_seq)
        #對Q、K、V做線性變換,一共做nb_head次，每次線性變化成size_per_head維度
        Q_seq = K.dot(Q_seq, self.WQ)
        Q_seq = K.reshape(Q_seq, (-1, K.shape(Q_seq)[1], self.nb_head, self.size_per_head))
        Q_seq = K.permute_dimensions(Q_seq, (0,2,1,3))#相當於transpose,排序各維度的順序
...
class TargetedDropout(keras.layers.Layer): #Targeted Dropout層
