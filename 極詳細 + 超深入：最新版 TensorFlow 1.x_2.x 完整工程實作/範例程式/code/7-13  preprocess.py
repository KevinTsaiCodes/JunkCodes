...
def read_item_index_to_entity_id_file():
    file = 'data/' + DATASET + '/item_index2entity_id.txt'#實際的業務項目（item）  索引（entity id）
    print('reading item index to entity id file: ' + file + ' ...')
    i = 0
    for line in open(file, encoding='utf-8').readlines():
        item_index = line.strip().split('\t')[0]
        satori_id = line.strip().split('\t')[1]
        item_index_old2new[item_index] = i#item與行的對應關系（重新編號）
        entity_id2index[satori_id] = i #entity id與行的對應關系（重新編號）
        i += 1

#產生ratings_final.txt  使用者 電影  喜歡/不關心（1/0）作為標簽
def convert_rating():
    ...
    for line in open(file, encoding='utf-8').readlines()[1:]:
        ...
        if rating >= THRESHOLD[DATASET]:#用來區分正向，負向
