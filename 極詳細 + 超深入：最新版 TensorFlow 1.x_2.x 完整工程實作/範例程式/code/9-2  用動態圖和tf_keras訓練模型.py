...
preimgdata = __import__("9-3  利用Resnet進行樣本預先處理")
...
##########
#建構模型
class DNN_Encoder(tf.keras.Model):#解碼器模型
    def __init__(self, embedding_dim):
        super(DNN_Encoder, self).__init__()
        #keras的全連線支援多維輸入。僅對最後一維進行處理
...
class BahdanauAttention(tf.keras.Model):
  ...
  def call(self, features, #features形狀(batch_size, 49, embedding_dim)
...
class RNN_Decoder(tf.keras.Model):
  ...
  def call(self, x, features, hidden):
    ...
    output, state = self.gru(x)#使用循環網路gru進行處理
...
def loss_function(real, pred):
    mask = 1 - np.equal(real, 0)#批次中被補0的序列不參與計算loss
...
for epoch in range(EPOCHS):
    start = time.time()
    total_loss= train_one_epoch(dataset=dataset,epoch=epoch,**model_objects)#訓練一次

    loss_plot.append(total_loss )#儲存loss
