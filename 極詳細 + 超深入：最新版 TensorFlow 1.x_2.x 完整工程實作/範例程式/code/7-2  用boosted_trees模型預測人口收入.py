...
LOSS_PREFIX = {'wide': 'linear/', 'deep': 'dnn/','BoostedTrees':'BoostedTrees/'}#定義模型的前綴
...
def export_model(model, model_type, export_dir):                #定義函數export_model ，用於匯出模型
  ...
  參數:
    ...
    export_dir: 匯出模型的路徑.
  """
  wide_columns, deep_columns ,boostedtrees_columns= build_model_columns()        #獲得列張量
  if model_type == 'wide':
...
class WideDeepArgParser(argparse.ArgumentParser):               #定義WideDeepArgParser類別，用於解析參數
  ...
  def __init__(self):                                           #起始化函數
    ...
    self.add_argument(
        '--model_type', '-mt', type=str, default='BoostedTrees',   #加入一個啟動參數--model_type，預設值為wide_deep
        choices=['wide', 'deep', 'wide_deep',"BoostedTrees"],                  #定義該參數的可選值
