...
if LANG=='ch':
    ...
    with open('中文標簽.csv','r+') as f: 		#開啟檔案				
        ...
        print(len(labelnames),type(labelnames),labelnames[:5]) #顯示輸出中文標簽    
else: 
    from datasets import imagenet
    labelnames = imagenet.create_readable_names_for_imagenet_labels() #獲得資料集標簽
    print(len(labelnames),labelnames[:5])                                     #顯示輸出標簽
...
sess = tf.InteractiveSession() #建立階段
init_fn(sess)                         #載入模型
...
assign_op = tf.assign(input_imgs, x)  #到目前為止input_imgs還沒有起始化，必須呼叫input_imgs.initializer或tf.assign為其給予值
sess.run( assign_op, feed_dict={x: img})

below = input_imgs - 8.0/255.0  #定義圖片的變化範圍
above = input_imgs + 8.0/255.0

belowv,abovev = sess.run( [below,above])

plt.imshow(floatarr_to_img(belowv))#人眼驗證
plt.show()
...
for i in range(demo_steps):
    ...
    if (i+1) % 10 == 0:
        ...
        if loss_value<0.02:    #提前結束
            break
   
adv = input_imgs.eval() #取得圖片
