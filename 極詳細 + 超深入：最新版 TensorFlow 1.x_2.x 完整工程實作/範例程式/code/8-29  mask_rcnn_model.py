...
# 由於MNS算法的版本不同，鎖定在 TensorFlow 1.8+ 
from distutils.version import LooseVersion
assert LooseVersion(tf.__version__) >= LooseVersion("1.8")

#定義全局輸入圖片大小（二選一）,圖片會被下取樣6次。必須能夠被2的6次方整除
IMAGE_MIN_DIM = 128
IMAGE_MAX_DIM = 128
#每個錨點的邊長初值
RPN_ANCHOR_SCALES = (8, 16, 32, 64, 128) 

#在訓練過程中，將選取多少個ROI放到fpn層
...
IMAGE_DIM = IMAGE_MAX_DIM  #選取1024
IMAGE_RESIZE_MODE = "square"#統一成IMAGE_MAX_DIM

# 圖片resize時，定義的最小的縮放範圍.0代表不進行最小縮放範圍限制
IMAGE_MIN_SCALE = 0

BACKBONE = "resnet101"     #主干網路使用resnet

#骨干網傳回的每一層特征，對原始圖片的拉遠比例.代表著輸出特征的5種尺度
#在計算錨點時，BACKBONE_STRIDES的每個元素，代表按照該像素值劃分網格，
#骨干網的輸出形狀即為256 128 64 32 16,代表輸出的網格個數為256 128 64 32 16
BACKBONE_STRIDES = [4, 8, 16, 32, 64]

#掃描網格的步長。按照該步長取得網格，用於計算錨點。網格中的第一個像素座標被當作錨點的中心點
RPN_ANCHOR_STRIDE = 1




#錨點的邊長比例(width/height)，將初值和邊長比例一起計算，得到錨點的真實邊長。
...
#對應與訓練或是使用時，RPN網路最終需要最大保留多少個ROI
POST_NMS_ROIS_TRAINING = 2000
POST_NMS_ROIS_INFERENCE = 1000
RPN_NMS_THRESHOLD = 0.7
FPN_FEATURE = 256 #特征金字塔層的深度
DETECTION_MAX_INSTANCES = 100#fpn最終檢驗的案例個數
#在製作樣本的標簽時，從一張圖中，最多只讀取100個案例
MAX_GT_INSTANCES = 100
#分類別時的置信度設定值
DETECTION_MIN_CONFIDENCE = 0.7
#檢驗時的Non-maximum suppression設定值
DETECTION_NMS_THRESHOLD = 0.3

# Pooled ROIs
POOL_SIZE = 7#金字塔對齊池化後的ROI形狀
MASK_POOL_SIZE = 14
MASK_SHAPE = [28, 28]


#RPN和最終檢驗的邊界框細化標准偏差 Bounding box refinement standard deviation for RPN and final detections.
RPN_BBOX_STD_DEV = np.array([0.1, 0.1, 0.2, 0.2])
BBOX_STD_DEV = np.array([0.1, 0.1, 0.2, 0.2])

#是否對遮罩進行壓縮
USE_MINI_MASK = True
MINI_MASK_SHAPE = (56, 56)  # 壓縮後的遮罩大小(height, width)

USE_RPN_ROIS=True #是否使用RPN結果（不適用時，是除錯場景，手動輸入）
...
#resnet中的identity_block(不帶卷冊積的短連結):kernel_size為第二層卷冊積核大小。filters為每層卷冊積核個數.stage和block用於命名
def identity_block(input_tensor, kernel_size, filters, stage, block, use_bias=True, train_bn=True):
    
    nb_filter1, nb_filter2, nb_filter3 = filters#解析出每層卷冊積核個數
    conv_name_base = 'res' + str(stage) + block + '_branch'#為卷冊積層命名
    bn_name_base = 'bn' + str(stage) + block + '_branch'#為BN層命名
    ...
    x = KL.Add()([x, input_tensor])#短連結
    x = KL.Activation('relu', name='res' + str(stage) + block + '_out')(x)
    return x

#resnet中的conv_block(帶卷冊積的短連結):strides為第一層的步長 ，進行了下取樣,所以帶卷冊積的短連結時也得下取樣
...
#群組建殘差網路，支援resnet50 or resnet101 兩種。  stage5是否將第5特征層的結果輸出
...
#  MaskRCNN Class
############################################################
class MaskRCNN():    #內定封裝的keras_model為真正模型
    def __init__(self, mode, model_dir,num_class,batch_size):#起始化
        """
        mode: 可以是 "training" 或 "inference" 兩種模式
        model_dir: 儲存模型的路徑
        """
    ...
    def mold_inputs(self, images):#輸入圖片預先處理
        molded_images = []
        image_metas = []
        windows = []
        for image in images:
    ...
    def build(self, mode):#建構Mask R-CNN架構

        #檢查尺寸合法性
        h, w = IMAGE_DIM,IMAGE_DIM;
        if h / 2**6 != int(h / 2**6) or w / 2**6 != int(w / 2**6):
            raise Exception("必須要被2的6次方整除.例如： 256, 320, 384, 448, 512, ... etc. ")
         
        input_image = KL.Input( shape=[None, None, 3], name="input_image")#定義輸入節點
        
        if mode == "inference":
            input_anchors = KL.Input(shape=[None, 4], name="input_anchors")#將全局的錨點框輸入
        elif mode == "training":#處理訓練接口
            #由圖片和標注產生的關於錨點的值1 -1 0.用於RPN的標簽
            input_rpn_match = KL.Input( shape=[None, 1], name="input_rpn_match", dtype=tf.int32)
            #由圖片和標注產生的關於錨點框的偏移量x,y 高寬縮放.用於RPN的標簽
    ...
    # Attach 3x3 conv to all P layers to get the final feature maps.
        ...
        rpn_feature_maps = [P2, P3, P4, P5, P6]#用於rpn使用,P5是最全的特征，對p5進行下取樣產生p6
        mrcnn_feature_maps = [P2, P3, P4, P5]#用於classifier heads 使用
        
        ###########################################
        #準備RPN網路
        # RPN Model 該模型會產生前後景 前後景機率 box  output_names
        rpn = build_rpn_model(RPN_ANCHOR_STRIDE, len(RPN_ANCHOR_RATIOS), FPN_FEATURE)#每個尺度的特征都是256
        # Loop through pyramid layers
        layer_outputs = []  # list of lists
        for p in rpn_feature_maps:
            layer_outputs.append(rpn([p]))#將resnet特征分別放到rpn網路中，輸出放到layer_outputs裡面
...
###################################################
        #下面數字意義：image_id=1 original_image_shape=3 image_shape=3 座標=4 縮放=1
        img_meta_size = 1 + 3 + 3 + 4 + 1 + self.num_class 		#定義圖片附加訊息
 
        input_image_meta = KL.Input(shape=[img_meta_size], name="input_image_meta")#定義圖片附加訊息
        
        #fpn網路對rpn_rois區域與特征資料 mrcnn_feature_maps進行計算。識別出分類別、邊框和遮罩
        if mode == "inference":
            # Network Heads
            # 對rpn_rois區域內的mrcnn_feature_maps做分類別，並微調box  Proposal classifier and BBox regressor heads
            #mrcnn_class分類別  mrcnn_bbox中心點長寬變化量
            mrcnn_class_logits, mrcnn_class, mrcnn_bbox =\
                fpn_classifier_graph(rpn_rois, mrcnn_feature_maps, input_image_meta,
                                     POOL_SIZE, self.num_class ,self.batch_size,
                                     train_bn=False,#不用bn
                                     fc_layers_size=1024)#全連線層1024個節點


            #輸出標准化座標 [batch, num_detections, (y1, x1, y2, x2, class_id, score)]
            #將rpn_rois與mrcnn_class, mrcnn_bbox群組合起來，算出真實的box
            detections = DetectionLayer( batch_size= self.batch_size, name="mrcnn_detection")(
                [rpn_rois, mrcnn_class, mrcnn_bbox, input_image_meta])

            #像素分割
            detection_boxes = KL.Lambda(lambda x: x[..., :4])(detections)#取出box座標
            mrcnn_mask = build_fpn_mask_graph(detection_boxes, mrcnn_feature_maps,
                                              input_image_meta,MASK_POOL_SIZE,#14
                                              self.num_class,self.batch_size,train_bn=False)#不用bn

            model = KM.Model([input_image, input_image_meta, input_anchors],#輸入
        [detections, mrcnn_class, mrcnn_bbox,mrcnn_mask, rpn_rois, rpn_class, rpn_bbox],#輸出
                             name='mask_rcnn')
        else:
            #獲得輸入資料的類別
...
#            from keras.engine import topology as saving
    ...
    def train(self, train_dataset, val_dataset,batch_size, learning_rate, epochs, layers,
        ...
        self.compile(learning_rate, LEARNING_MOMENTUM)#設定模型的改善器及研讀參數
    ...
    def detect(self, images, verbose=0):#使用模型進行檢驗
        """使用模型進行檢驗
        輸入： images
        輸出：字典型態。內含如下內容
        rois: 檢驗框[N, (y1, x1, y2, x2)]
        class_ids: 類別別[N]
        scores: 分數[N]
        masks: 遮罩[H, W, N] 
        """
...
def build_rpn_targets(image_shape, anchors, gt_class_ids, gt_boxes):
    ...
    rpn_match: [N] (int32) matches between anchors and GT boxes.
               1 = positive anchor, -1 = negative anchor, 0 = neutral（iou在0.3和0.7之間）
    rpn_bbox: [N, (dy, dx, log(dh), log(dw))] Anchor bbox deltas.
    """
    # RPN Match: 1 = positive anchor, -1 = negative anchor, 0 = neutral
    rpn_match = np.zeros([anchors.shape[0]], dtype=np.int32)
    # RPN bounding boxes: [max anchors per image, (dy, dx, log(dh), log(dw))]
    #其實rpn_bbox可以砍掉一般。因為只有放了一半的正錨點
    ...
    # Compute overlaps [num_anchors, num_gt_boxes]  每個值都是面積重疊的比例
    ...
    rpn_match[(anchor_iou_max < 0.3) & (no_crowd_bool)] = -1#將Iou《0.3的設為-1
    # 2. Set an anchor for each GT box (regardless of IoU value).
    # TODO: If multiple anchors have the same IoU match all of them
    gt_iou_argmax = np.argmax(overlaps, axis=0)
    rpn_match[gt_iou_argmax] = 1#將錨點中，對應與任何一個bbox的Iou最大的值都設為1
    # 3. Set anchors with high overlap as positive.
    rpn_match[anchor_iou_max >= 0.7] = 1#將將Iou>= 0.7的設為
    #要麼充分重疊，要麼不充分重疊。  介於二者之前的都是0

    # Subsample to balance positive and negative anchors
    # Don't let positives be more than half the anchors
    #總共256個正負錨點框。正負錨點有超過半數的，透過隨機值將其去掉。
...
def generate_random_rois(image_shape, count, gt_class_ids, gt_boxes):
    ...
    rois_per_box = int(0.9 * count / gt_boxes.shape[0])#計算每個bbox需要產生的隨機框
    for i in range(gt_boxes.shape[0]):
        ...
        #在gt_boxes範圍內產生rois_per_box個隨機框。要求邊長都得大於threshold
        #若果一次每產生對。就再來一次，直到產生對為止
        while True:
            y1y2 = np.random.randint(r_y1, r_y2, (rois_per_box * 2, 2))
            x1x2 = np.random.randint(r_x1, r_x2, (rois_per_box * 2, 2))
            # Filter out zero area boxes
            threshold = 1
            #從邊長大於1的框裡面取出rois_per_box個。若果不足，shape[0]將小於rois_per_box
    ...
    rois[-remaining_count:] = global_rois#將最後的隨機填入
    return rois
...
def data_generator(dataset,  shuffle=True, augment=False, augmentation=None,
    ...
    image_index = -1
    image_ids = np.copy(dataset.image_ids)#所有圖片的id陣列
    error_count = 0
    ...
    while True:
        try:
            ...
            image_id = image_ids[image_index]#取一個索引ID
...
#                                use_mini_mask=USE_MINI_MASK)
            ...
            # Mask R-CNN Targets
            if random_rois:
                #根據bbox和image，產生了random_rois個隨機框
...
class DetectionTargetLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        #按照批次內定的圖片，依次呼叫detection_targets_graph進行最終結果過濾
